...
# Define the Perceptron model for the AND gate
...
input_size = 2  # AND gate has two input features
...
labels = torch.tensor([[0.0], [0.0], [0.0], [1.0]])  # Expected outputs
...
# Testing the trained model on AND gate inputs
model.eval()  # Set the model to evaluation mode
with torch.no_grad():
    test_output = model(data)
    predicted = test_output.round()  # Round to get binary output
    print(f'Predicted outputs for AND gate:\n{predicted}')
